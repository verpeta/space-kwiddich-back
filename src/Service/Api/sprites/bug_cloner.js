...
const { Bug } = require('./items')

class BugCloner extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, entityID, x = -100, y = -100, maxCowCount = 5, cloneDelay = 4000) {
        ...
        this.type = SpriteType.BUG_CLONER
    ...
    update(time) {
        ...
        if (this.timeFromLastClone + this.cloneDelay <= time) {
            ...
            // Add cow to game
            const cow = new Bug(this.scene, this.scene.getID(), this.x, this.y - this.height * 3)
